import { swarmInputSchema, decideSwarmWithJev, swarmJevConfiguration, SwarmJevError } from "../../../lib/swarm-jev";
...
async function readBody(request: Request): Promise<unknown> {
  if (Number(request.headers.get("content-length") || "0") > MAX_BODY_BYTES) {
    throw new SwarmJevError(413, "That observation is too large. Send only the bounded recent memory.");
  }
  if (!request.body) throw new SwarmJevError(400, "Send the current simulator observation and memory.");
  ...
  try {
    ...
    throw new SwarmJevError(400, "The request is not valid JSON.");
  } finally {
    reader.releaseLock();
  }
}
...
export async function GET() {
  return json(swarmJevConfiguration());
}

export async function POST(request: Request) {
  try {
    if (!hasSameOrigin(request)) {
      return json({ error: "Send swarm requests from the simulator's own page." }, 403);
    }
    if (!/^application\/json(?:\s*;|$)/i.test(request.headers.get("content-type") || "")) {
      return json({ error: "Use application/json for swarm requests." }, 415);
    }
    if (!consumeRequest(request)) {
      return json({ error: "Too many swarm decisions. Wait a moment, then try again." }, 429, { "Retry-After": "60" });
    }

    const input = swarmInputSchema.safeParse(await readBody(request));
    if (!input.success) {
      return json({ error: "Send a valid bounded simulator observation and memory." }, 400);
    }
    return json(await decideSwarmWithJev(input.data, { signal: request.signal }));
  } catch (error) {
    if (error instanceof SwarmJevError) return json({ error: error.message }, error.status);
    return json({ error: "The simulator could not process that observation. The swarm is stopped." }, 500);
  }
}
