import { simulatorInputSchema, decideSimulatorWithJev, simulatorJevConfiguration, SimulatorJevError } from "../../../lib/simulator-jev";
...
async function readBody(request: Request): Promise<unknown> {
  if (Number(request.headers.get("content-length") || "0") > MAX_BODY_BYTES) {
    throw new SimulatorJevError(413, "That message is too large. Try a shorter one.");
  }
  if (!request.body) throw new SimulatorJevError(400, "Send a message and the simulator's current context.");
  ...
  try {
    ...
    throw new SimulatorJevError(400, "The request is not valid JSON.");
  } finally {
    reader.releaseLock();
  }
}
...
export async function GET() {
  return json(simulatorJevConfiguration());
}

export async function POST(request: Request) {
  try {
    if (!hasSameOrigin(request)) {
      return json({ error: "Send messages from the simulator's own page." }, 403);
    }
    if (!/^application\/json(?:\s*;|$)/i.test(request.headers.get("content-type") || "")) {
      return json({ error: "Use application/json for simulator requests." }, 415);
    }
    if (!consumeRequest(request)) {
      return json({ error: "A few too many messages. Wait a moment, then try again." }, 429, { "Retry-After": "60" });
    }

    const input = simulatorInputSchema.safeParse(await readBody(request));
    if (!input.success) {
      return json({ error: "Send a message of 1–500 characters and valid simulator context." }, 400);
    }
    return json(await decideSimulatorWithJev(input.data));
  } catch (error) {
    if (error instanceof SimulatorJevError) return json({ error: error.message }, error.status);
    return json({ error: "The simulator could not process that message. Please try again." }, 500);
  }
}
