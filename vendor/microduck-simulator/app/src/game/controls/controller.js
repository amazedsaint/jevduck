...
export class Controller {
  ...
  update(dt) {
    ...
    if (this.#neutralRelease?.()) this.#neutralRelease = null;
  ...
  // A stop must not spring back into motion while a key or stick remains
  // held. This independent input gate leaves ceremony locks untouched.
  holdUntilNeutral(isNeutral) {
    this.#neutralRelease = isNeutral() ? null : isNeutral;
  }

  get neutralHeld() {
    return this.#neutralRelease !== null;
  }
  ...
  getCommand() {
    if (this.#locked || this.#neutralRelease) return ZERO_CMD;
