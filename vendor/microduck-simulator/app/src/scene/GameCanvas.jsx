...
import { EMBEDDED } from "../game/embedded.js";
...
function Game() {
  ...
  return (
    <>
      <ambientLight name="sim-ambient" intensity={0.6} />
      <directionalLight name="sim-key" position={[2, 4, 2]} intensity={1.6} />
      <directionalLight name="sim-fill" position={[-2, 2, 1.5]} intensity={0.4} />
      <directionalLight name="sim-rim" color={0xffb366} position={[0, 3, -2]} intensity={0.7} />
    </>
  );
}
...
export default function GameCanvas() {
  return (
    <Canvas
      ...
      dpr={[1, 2]}
      // Embedded worlds already clear opaque; skip page-alpha compositing.
      gl={{ antialias: true, alpha: !EMBEDDED }}
    ...
    >
      <Game />
      {!EMBEDDED && CRT_DISTORTION_ENABLED && <CrtDistortion />}
    </Canvas>
  );
}
